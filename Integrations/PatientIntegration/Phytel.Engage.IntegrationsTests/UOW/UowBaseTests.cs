...
namespace Phytel.Engage.Integrations.UOW.Tests
{
    ...
    public class UowBaseTests
    {
        ...
        [TestMethod()]
        public void InitPCPPhonesTest()
        {
            InitializeMappings();
            var pi = new UowBase();
            var RepositoryFactory = new RepositoryFactory();
            pi.InitPCPPhones("HILLCREST001", RepositoryFactory);
            Assert.IsNotNull(pi.PCPPhones);
        }

        [TestMethod()]
        public void InitPatientSystems()
        {
            InitializeMappings();
            var pi = new UowBase();
            var RepositoryFactory = new RepositoryFactory();
            pi.InitPatientSystems("HILLCREST001", RepositoryFactory);
            Assert.IsNotNull(pi.PatientSystems);
        }

        [TestMethod()]
        public void InitPatientNotes()
        {
            InitializeMappings();
            var pi = new UowBase();
            var RepositoryFactory = new RepositoryFactory();
            pi.InitPatientNotes("ORLANDOHEALTH001", RepositoryFactory);
            Assert.IsNotNull(pi.PatientNotes);
        }
    }
}

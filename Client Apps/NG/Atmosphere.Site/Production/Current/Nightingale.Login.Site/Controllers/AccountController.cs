...
using Nightingale.Login.Site.Filters;
using Nightingale.Login.Site.Models;
using C3.Business;
using C3.Web.Helpers;
using C3.Data;
using Atmosphere.Web;

namespace Nightingale.Login.Site.Controllers
{
    [Authorize]
    ...
    public class AccountController : Controller
    {
        ...
        {
            returnUrl = string.IsNullOrEmpty(returnUrl) ? returnUrl : "/NightingaleUI";
            ...
            returnUrl = Request["ReturnURL"] == null ? returnUrl : Request["ReturnURL"];

            if (string.IsNullOrEmpty(returnUrl))
            returnUrl = user.Controls.Count == 0 ? "~/AccessDenied.aspx" : user.Controls[0].Path;
            ...
            PageInfo info = UserHelper.RedirectUser(user, returnUrl, "home", false);

            // Auditing services is throwing a null reference exception
            ...
            if (user.LoginToken.Trim() != string.Empty)
                info.RedirectURL += "/#ut/" + user.LoginToken;

            return Redirect(info.RedirectURL);
            //return RedirectToAction(info.RedirectURL);

    ...
    function (models, servicesConfig, datacontext, session) {
        ...
        ctor.prototype.addNewObservation = function () {
            ...
            if (matchedObservation) {
                datacontext.initializeObservation(newObservation, 'Observation', matchedObservation.id(), matchedObservation.typeId(), self.selectedPatient().id()).then(dataReturned);
            }
        ...
        ctor.prototype.activate = function (settings) {
            ...
            self.getAdditionalObservationsById = function () {
                if (self.activeDataType()) {
                    ...
                    ko.utils.arrayForEach(theseObservations, function (obs) {
                        if ((obs.typeId() === self.activeDataType().id()) && (obs.standard() === false)) {
            ...
            self.addNew = function (sender) {
                ...
                if (moment(sender.startDate()).isValid() && sender.computedValue().value()) {
                    ...
                    return datacontext.initializeObservation(newObservation, 'Observation', sender.observationId(), sender.typeId(), sender.patientId()).then(dataReturned);
                }
                function dataReturned() {
        ...
        ctor.prototype.getAdditionalObservations = function () {
            var self = this;
            if (self.selectedPatient() && self.activeDataType()) {
                var endPoint = new servicesConfig.createEndPoint('1.0', session.currentUser().contracts()[0].number(), 'patient/' + self.selectedPatient().id() + '/Observation/Type/' + self.activeDataType().id() + '/MatchLibrary/', 'AdditionalObservation');
                return datacontext.getEntityList(self.additionalObservations, 'AdditionalObservation', endPoint.ResourcePath, 'typeId', self.activeDataType().id(), false).then(dataReturned);
            }
            function dataReturned() {
                // Reset the search engine's cache

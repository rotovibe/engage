define([],
    function () {
        ...
        ko.bindingHandlers.datepicker = {
            init: function (element, valueAccessor, allBindingsAccessor, bindingContext) {
                ...
                ko.utils.registerEventHandler(element, "change", function () {
					if( !initialized ){
						//ignore the first date change that is set by init:
						initialized = true;
						return;
					}
                    // Get the new date
					...
					if( observableMoment.isValid() ){
						if( !isSameDate(datepickerMoment, observableMoment) ){
							//console.log('datepicker registerEventHandler calling setDateValue src = ' +datepickerMoment.format() + ' dest='+ observableMoment.format());
					...
					//console.log('datepicker registerEventHandler set observable= ' + observableMoment.format());	
                    observable(observableMoment.format());//toISOString
                });								
		...
		function setDateValue( momentSrc, momentDest ){
			//console.log('datepicker setDateValue starts: src='+ momentSrc.format() + ' dest=' +momentDest.format());
			momentDest.date( momentSrc.date() );
			momentDest.month( momentSrc.month() );
			momentDest.year( momentSrc.year() );
			//console.log('datepicker setDateValue returns: ' + momentDest.format());
			return momentDest;
		}
        ...
        ko.bindingHandlers.jqtimepicker = {
            init: function (element, valueAccessor, allBindingsAccessor, bindingContext) {
				...
				if( observable && observable() && moment( observable() ).isValid() ){
					...
					//console.log('jqtimepicker init setting the timepicker to the observable: ' + observable());
					thisElement.timepicker().setTime(observableMoment.hours() + ':' + observableMoment.minutes());
				}
				else{
					initialized = true;
				}
				...
				function onTimeChange(time){
					...
					if(time){										
						...
						if( !timepickerMoment.isValid() ){
							...
							timeMoment.utc();
							newHourUTC = timeMoment.hours(); //time.getHours();
							newMinutesUTC = timeMoment.minutes();//time.getMinutes();														
						}
					}
					...
					setTimeValue(observable, newHourUTC, newMinutesUTC);					
					// console.log('jqtimepicker onTimeChange ends: time=' + time + ' observable =' + observable());
				}
				function setTimeValue(dateObservable, hourUTC, minuteUTC){					
					var observableMoment = moment.utc(dateObservable());
					// console.log('jqtimepicker setTimeValue starts with dateObservable=' + observableMoment.format());
					if(observableMoment.isValid()){
						if(observableMoment.hours() !== hourUTC || observableMoment.minutes() !== minuteUTC){
							observableMoment.hours(hourUTC).minutes(minuteUTC).seconds(0);
							dateObservable(observableMoment.format());	//toISOString() //note - moment accounts for DST :)
							// console.log('jqtimepicker setTimeValue set observable=' + observableMoment.format());
						}						
					}
				}
			}

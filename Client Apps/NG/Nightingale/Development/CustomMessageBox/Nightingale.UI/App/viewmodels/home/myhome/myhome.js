define(['services/session', 'services/datacontext', 'config.services', 'viewmodels/shell/shell', 'models/base', 'services/local.collections', 'services/navigation', 'plugins/router'],
    function (session, datacontext, servicesConfig, shell, modelConfig, localCollections, navigation, router) {
        ...
        function CalendarOptionsModel(events, header, editable, viewDate, defaultView) {
            ...
            self.eventClick = function (event) {
                ...
                if (navigation.currentRoute() && navigation.currentRoute().config.title === 'Individual') {
                    ...
                    // Get the event
                    var thisEvent = datacontext.getEventById(event.id, false);
        ...
        var myToDos = ko.computed(function () {
            ...
            orderString = selectedTodoSortColumn() ? selectedTodoSortColumn() : 'dueDate desc';
            ...
            // Add an event for each todo
            ko.utils.arrayForEach(theseTodos, function (todo) {
                if (moment(todo.dueDate()).isValid()) {
                    // Create an event
                    var thisEvent = new Event();
                    thisEvent.id(todo.id());
                    thisEvent.title((todo.patientDetails() ? todo.patientDetails().fullLastName() + ', ' + todo.patientDetails().fullFirstName() + ' - ' : '') + todo.title());
                    thisEvent.start(todo.dueDate());
                    thisEvent.allDay(true);
                    thisEvent.patientId(todo.patientId());
                    thisEvent.patientName((todo.patientDetails() ? todo.patientDetails().fullLastName() + ', ' + todo.patientDetails().fullFirstName() : '-'));
                    // Find the current users preferred name
                    ...
                    thisEvent.assignedToName(thisMatchedCareManager.preferredName());
                    thisEvent.typeId(2);
                    addEvent(thisEvent);
                }
            });

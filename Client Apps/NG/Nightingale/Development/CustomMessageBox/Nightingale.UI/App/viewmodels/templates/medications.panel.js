...
define(['services/session', 'services/datacontext', 'config.services', 'viewmodels/shell/shell', 'models/base', 'viewmodels/patients/medications/index', 'durandal/app'],
    function (session, datacontext, servicesConfig, shell, modelConfig, medicationsIndex, app) {
    	...
    	var allColumns = [
    		...
    		new Column('status', 'Active', 'span2 ellipsis', 'statusId'),
            ...
            new Column('strength', 'Strength','span3 ellipsis', 'stength.name'),
            new Column('strength-small', 'Strength','span2 ellipsis', 'stength.name'),
...
/*					//wire delete function
					if(!td.deletePatientMedication) {
						td.deletePatientMedication = function(){
							self.deletePatientMedication(td);
						}
					}*/
    			});
                return self.medications();
    		});
/*			self.deletePatientMedication = function(medication){
				// Prompt the user to confirm deletion
				var message = 'You are about to delete: ' + medication.name() +' from this patient.  Press OK to continue, or cancel to return without deleting.';                
				
				// // this does not show a messagebox. it does add it to the html BUT since durandal.css doeos not exist it wont show!
				// app.showMessage(message, 'Delete Confirmation', ['Yes', 'No'])

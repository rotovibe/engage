define(['models/users', 'config.services', 'services/local.collections'],
    function (userModelsConfig, serviceConfig, localCollections) {
        ...
        var getEventById = function (manager, eventId, forceRemote) {
            ...
            // If it is not forceRemote, then try to get this from the local cache
            if (!forceRemote) {
                var p = getLocalById(manager, 'local', 'Event', 'id', eventId);  //Go check and see if we have this locally already
                if (p.length > 0) {
                    return p[0];
                }
            }

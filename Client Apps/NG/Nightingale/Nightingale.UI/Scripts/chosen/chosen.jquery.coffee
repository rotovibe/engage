...
class Chosen extends AbstractChosen
  ...
  results_show: ->
    ...
    parent = @dropdown.removeClass('above-container').closest('.tab,.content')
    parent_bottom = parent.outerHeight() + parent.offset().top
    dropdown_bottom = @dropdown.height() + @dropdown.offset().top
    
    if parent_bottom < dropdown_bottom
      @dropdown.addClass('above-container')
  ...
  search_results_mouseup: (evt) ->
    target = if $(evt.target).hasClass("active-result") or $(evt.target).hasClass("result-selected") then $(evt.target) else $(evt.target).parents(".active-result").first()
    if target.length
      if target.hasClass "result-selected"
        @choice_destroy(target)
      else
  ...
  search_results_mouseout: (evt) ->
    this.result_clear_highlight() if $(evt.target).hasClass("active-result") or $(evt.target).hasClass("result") or $(evt.target).parents('.active-result').first()
  ...
  update_selection_count: ->
    count = @results_data.filter((el) -> el.selected is true).length
    @search_field.val if count is 0 then "None selected" else "#{count} selected"
  ...
  result_select: (evt) ->
    if @result_highlight
      ...
      this.results_hide() unless @is_multiple
  ...
  keydown_arrow: ->
    if @results_showing and @result_highlight
      next_sib = @result_highlight.nextAll("li[class!=disabled-result]").first()
  ...
  keyup_arrow: ->
    if not @results_showing and not @is_multiple
      ...
      prev_sibs = @result_highlight.prevAll("li[class!=disabled-result]")

(function() {
  ...
  Chosen = (function(_super) {
    ...
    Chosen.prototype.results_show = function() {
      var dropdown_bottom, parent, parent_bottom;
      ...
      this.search_field.val(this.search_field.val());
      this.winnow_results();
      parent = this.dropdown.removeClass('above-container').closest('.tab,.content');
      console.log('Parent - ' + parent);
      console.log('this dropdown - ' + this.dropdown);
      console.log('Parent outer height - ' + parent.outerHeight());
      console.log('Parent offset top - ' + parent.offset().top);
      console.log('This dropdown height - ' + this.dropdown.height());
      console.log('This dropdown offset top - ' + this.dropdown.offset().top);
      parent_bottom = parent.outerHeight() + parent.offset().top;
      dropdown_bottom = this.dropdown.height() + this.dropdown.offset().top;
      if (parent_bottom < dropdown_bottom) {
        return this.dropdown.addClass('above-container');
      }
    };
    ...
    Chosen.prototype.search_results_mouseup = function(evt) {
      var target;
      target = $(evt.target).hasClass("active-result") || $(evt.target).hasClass("result-selected") ? $(evt.target) : $(evt.target).parents(".active-result").first();
      if (target.length) {
        if (target.hasClass("result-selected")) {
          return this.choice_destroy(target);
        } else {
    ...
    Chosen.prototype.search_results_mouseout = function(evt) {
      if ($(evt.target).hasClass("active-result") || $(evt.target).hasClass("result") || $(evt.target).parents('.active-result').first()) {
    ...
    Chosen.prototype.update_selection_count = function() {
      var count;
      count = this.results_data.filter(function(el) {
        return el.selected === true;
      }).length;
      return this.search_field.val(count === 0 ? "None selected" : "" + count + " selected");
    };
    ...
    Chosen.prototype.keydown_arrow = function() {
      ...
      if (this.results_showing && this.result_highlight) {
        next_sib = this.result_highlight.nextAll("li[class!=disabled-result]").first();
        if (next_sib) {
    ...
    Chosen.prototype.keyup_arrow = function() {
      ...
      if (!this.results_showing && !this.is_multiple) {
        ...
        prev_sibs = this.result_highlight.prevAll("li[class!=disabled-result]");
        if (prev_sibs.length) {

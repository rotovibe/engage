...
AS
BEGIN
	TRUNCATE TABLE [RPT_BSHSI_HW2_Enrollment_Info]
	...
	SELECT DISTINCT 	
			...
			,(		COALESCE( 			(SELECT top 1 				CASE WHEN pr.[delete] = 'False' AND pa.[State] = 'Completed' 					THEN 						pr.[Text] 					ELSE 						NULL 					END   AS [Text] 			 FROM 				RPT_Patient AS ptss with (nolock)  				INNER JOIN RPT_PatientProgram AS pp with (nolock)  ON ptss.PatientId = pp.PatientId  				INNER JOIN RPT_PatientProgramModule AS pm with (nolock) ON pp.PatientProgramId = pm.PatientProgramId  				INNER JOIN RPT_PatientProgramAction AS pa with (nolock)  ON pm.PatientProgramModuleId = pa.PatientProgramModuleId  				INNER JOIN RPT_PatientProgramStep AS ps with (nolock) ON pa.ActionId = ps.ActionId  				LEFT OUTER JOIN RPT_PatientProgramResponse AS pr with (nolock) ON ps.StepId = pr.StepId 			WHERE 				pp.SourceId = '541943a6bdd4dfa5d90002da' 				AND ((ps.SourceId = '53f4fb39ac80d30e00000067')  OR (ps.SourceId = '532c3e76c347865db8000001') ) 				AND ((pa.SourceId = '53f4fd75ac80d30e00000083') OR (pa.SourceId = '532c45bff8efe36886000446') ) 				AND (pp.[Delete] = 'False') 				AND (pa.Archived = 'False') 				AND pr.Selected = 'True' 				AND ptss.PatientId = pt.PatientId 				AND pp.patientprogramid = ppt.patientprogramid) 		, 			(SELECT     TOP 1 				CASE WHEN pr.[delete] = 'False' AND pa.[State] = 'Completed' 					THEN 						pr.[Text] 					ELSE 						NULL 					END   AS [Text] 			 FROM 				RPT_Patient AS pt1 with (nolock)  				INNER JOIN RPT_PatientProgram AS pp1 with (nolock)  ON pt1.PatientId = pp1.PatientId  				INNER JOIN RPT_PatientProgramModule AS pm with (nolock) ON pp1.PatientProgramId = pm.PatientProgramId  				INNER JOIN RPT_PatientProgramAction AS pa with (nolock)  ON pm.PatientProgramModuleId = pa.PatientProgramModuleId  				INNER JOIN RPT_PatientProgramStep AS ps with (nolock) ON pa.ActionId = ps.ActionId  				LEFT OUTER JOIN RPT_PatientProgramResponse AS pr with (nolock) ON ps.StepId = pr.StepId 			 WHERE  				pp1.SourceId = '541943a6bdd4dfa5d90002da' 				AND ((ps.SourceId = '53f4fb39ac80d30e00000067')  OR (ps.SourceId = '532c3e76c347865db8000001') ) 				AND ((pa.SourceId = '53f4fd75ac80d30e00000083') OR (pa.SourceId = '532c45bff8efe36886000446') ) 				AND (pp1.[Delete] = 'False') 				AND pr.Selected = 'True' 				AND pt1.PatientId = pt.PatientId 				AND pp1.patientprogramid = ppt.patientprogramid 				AND pa.ActionId IN ( 					SELECT      DISTINCT 						pa.ActionId 					FROM 						RPT_Patient AS pt2 with (nolock)  						INNER JOIN RPT_PatientProgram AS pp2 with (nolock)  ON pt2.PatientId = pp2.PatientId  						INNER JOIN RPT_PatientProgramModule AS pm with (nolock) ON pp2.PatientProgramId = pm.PatientProgramId  						INNER JOIN RPT_PatientProgramAction AS pa with (nolock)  ON pm.PatientProgramModuleId = pa.PatientProgramModuleId  						INNER JOIN RPT_PatientProgramStep AS ps with (nolock) ON pa.ActionId = ps.ActionId  						LEFT OUTER JOIN RPT_PatientProgramResponse AS pr with (nolock) ON ps.StepId = pr.StepId 					WHERE 						pp2.SourceId = '541943a6bdd4dfa5d90002da' 						AND ((ps.SourceId = '53f4fb39ac80d30e00000067')  OR (ps.SourceId = '532c3e76c347865db8000001') ) 						AND ((pa.SourceId = '53f4fd75ac80d30e00000083') OR (pa.SourceId = '532c45bff8efe36886000446') ) 						AND (pp2.[Delete] = 'False') 						AND pr.Selected = 'True' 						AND pt2.PatientId = pt1.PatientId 						AND pp2.patientprogramid = ppt.patientprogramid 				) 				ORDER BY pa.ArchivedDate DESC) 		)) as [Market]
			,( 		COALESCE( 			(SELECT 				CASE WHEN pr3.[delete] = 'False' AND pa3.[State] = 'Completed' 						THEN 							CASE WHEN ISDATE(pr3.Value) = 1 THEN CAST(pr3.Value AS DATE) ELSE NULL END  					ELSE 						NULL 				END 						 				AS [DisEnrollValue] 			FROM          				RPT_Patient AS pt3 with (nolock) INNER JOIN 				RPT_PatientProgram AS pp3 with (nolock) ON pt3.PatientId = pp3.PatientId INNER JOIN 				RPT_PatientProgramModule AS pm3 with (nolock) ON pp3.PatientProgramId = pm3.PatientProgramId INNER JOIN 				RPT_PatientProgramAction AS pa3 with (nolock) ON pm3.PatientProgramModuleId = pa3.PatientProgramModuleId INNER JOIN 				RPT_PatientProgramStep AS ps3 with (nolock) ON pa3.ActionId = ps3.ActionId LEFT OUTER JOIN 				RPT_PatientProgramResponse AS pr3 with (nolock) ON ps3.StepId = pr3.StepId 			WHERE      			((ps3.SourceId = '53f56eb7ac80d31203000001') OR (ps3.SourceId = '532c4061f8efe368860003b6') ) 			AND ((pa3.SourceId = '53f57115ac80d31203000014') OR (pa3.SourceId = '532c46b3c347865db8000092')) 			AND pp3.Name = 'BSHSI - Healthy Weight v2' 			AND (pp3.[Delete] = 'False') AND (pa3.Archived = 'False') 			AND pr3.Value != '' 			AND pt3.patientid = pt.patientid 			AND pa3.[State] IN ( 'Completed') 			AND pp3.patientprogramid = ppt.patientprogramid) 		, 			(SELECT   TOP 1 				CASE WHEN pr4.[delete] = 'False' AND pa4.[State] = 'Completed' 						THEN 							CASE WHEN ISDATE(pr4.Value) = 1 THEN CAST(pr4.Value AS DATE) ELSE NULL END  					ELSE 						NULL 				END 						 				AS [DisEnrollValueArch]						 			FROM          				RPT_Patient AS pt4 with (nolock) INNER JOIN 				RPT_PatientProgram AS pp4 with (nolock) ON pt4.PatientId = pp4.PatientId INNER JOIN 				RPT_PatientProgramModule AS pm4 with (nolock) ON pp4.PatientProgramId = pm4.PatientProgramId INNER JOIN 				RPT_PatientProgramAction AS pa4 with (nolock) ON pm4.PatientProgramModuleId = pa4.PatientProgramModuleId INNER JOIN 				RPT_PatientProgramStep AS ps4 with (nolock) ON pa4.ActionId = ps4.ActionId LEFT OUTER JOIN 				RPT_PatientProgramResponse AS pr4 with (nolock) ON ps4.StepId = pr4.StepId 			WHERE      			((ps4.SourceId = '53f56eb7ac80d31203000001') OR (ps4.SourceId = '532c4061f8efe368860003b6') ) 			AND ((pa4.SourceId = '53f57115ac80d31203000014') OR (pa4.SourceId = '532c46b3c347865db8000092')) 			AND pp4.Name = 'BSHSI - Healthy Weight v2' 			AND (pp4.[Delete] = 'False') 			AND pr4.Value != '' 			AND pa4.[State] IN ('Completed') 			AND pa4.Archived = 'True' 			AND pt4.patientid = pt.patientid 			AND pp4.patientprogramid = ppt.patientprogramid 			AND pa4.ActionId IN ( 				SELECT		 					DISTINCT pa5.ActionId 				FROM          					RPT_Patient AS pt5 with (nolock) INNER JOIN 					RPT_PatientProgram AS pp5 with (nolock)  ON pt5.PatientId = pp5.PatientId INNER JOIN 					RPT_PatientProgramModule AS pm5 with (nolock) ON pp5.PatientProgramId = pm5.PatientProgramId INNER JOIN 					RPT_PatientProgramAction AS pa5 with (nolock)  ON pm5.PatientProgramModuleId = pa5.PatientProgramModuleId INNER JOIN 					RPT_PatientProgramStep AS ps5 with (nolock) ON pa5.ActionId = ps5.ActionId LEFT OUTER JOIN 					RPT_PatientProgramResponse AS pr5 with (nolock) ON ps5.StepId = pr5.StepId 				WHERE   					((ps5.SourceId = '53f56eb7ac80d31203000001') OR (ps5.SourceId = '532c4061f8efe368860003b6') ) 					AND ((pa5.SourceId = '53f57115ac80d31203000014') OR (pa5.SourceId = '532c46b3c347865db8000092')) 					AND pp5.Name = 'BSHSI - Healthy Weight v2' 					AND (pp5.[Delete] = 'False') 					AND pr5.Value != '' 					AND pa5.[State] IN ('Completed') 					AND pt5.PatientId = pt4.PatientId 					AND pp5.patientprogramid = ppt.patientprogramid 			) 			ORDER BY pa4.ArchivedDate DESC 			) 	)) as [Disenroll_Date]	 	
			,( 		COALESCE( 			(SELECT 				CASE WHEN pr6.[delete] = 'False' AND pa6.[State] = 'Completed' 						THEN 							pr6.[Text] 					ELSE 						NULL 				END   AS [ReasonText] 			FROM 				RPT_Patient AS pt6 with (nolock) INNER JOIN 				RPT_PatientProgram AS pp6 with (nolock) ON pt6.PatientId = pp6.PatientId INNER JOIN 				RPT_PatientProgramModule AS pm6 with (nolock) ON pp6.PatientProgramId = pm6.PatientProgramId INNER JOIN 				RPT_PatientProgramAction AS pa6 with (nolock) ON pm6.PatientProgramModuleId = pa6.PatientProgramModuleId INNER JOIN 				RPT_PatientProgramStep AS ps6 with (nolock) ON pa6.ActionId = ps6.ActionId LEFT OUTER JOIN 				RPT_PatientProgramResponse AS pr6 with (nolock) ON ps6.StepId = pr6.StepId 			WHERE      				((ps6.SourceId= '53f56f10ac80d31200000001') OR (ps6.SourceId= '532c407fc347865db8000003') ) 				AND ((pa6.SourceId = '53f57115ac80d31203000014') OR (pa6.SourceId = '532c46b3c347865db8000092')) 				AND pp6.Name = 'BSHSI - Healthy Weight v2' 				AND (pp6.[Delete] = 'False') AND (pa6.Archived = 'False') 				AND pr6.Selected = 'True' 				AND pa6.[State] IN ('Completed') 				AND pa6.Archived = 'False' 				AND pt6.patientid = pt.patientid 				AND pp6.patientprogramid = ppt.patientprogramid) 		, 			(SELECT   TOP 1   				CASE WHEN pr7.[delete] = 'False' AND pa7.[State] = 'Completed' 						THEN 							pr7.[Text] 					ELSE 						NULL 				END   AS [DisenrollReasonText] 			FROM 				RPT_Patient AS pt7 with (nolock) INNER JOIN 				RPT_PatientProgram AS pp7 with (nolock) ON pt7.PatientId = pp7.PatientId INNER JOIN 				RPT_PatientProgramModule AS pm7 with (nolock) ON pp7.PatientProgramId = pm7.PatientProgramId INNER JOIN 				RPT_PatientProgramAction AS pa7 with (nolock) ON pm7.PatientProgramModuleId = pa7.PatientProgramModuleId INNER JOIN 				RPT_PatientProgramStep AS ps7 with (nolock) ON pa7.ActionId = ps7.ActionId LEFT OUTER JOIN 				RPT_PatientProgramResponse AS pr7 with (nolock) ON ps7.StepId = pr7.StepId 			WHERE      				((ps7.SourceId= '53f56f10ac80d31200000001') OR (ps7.SourceId= '532c407fc347865db8000003') ) 				AND ((pa7.SourceId = '53f57115ac80d31203000014') OR (pa7.SourceId = '532c46b3c347865db8000092')) 				AND pp7.Name = 'BSHSI - Healthy Weight v2' 				AND (pp7.[Delete] = 'False')  				AND pr7.Selected = 'True' 				AND pa7.[State] IN ('Completed') 				AND pa7.Archived = 'True' 				AND pt7.patientid = pt.patientid 				AND pp7.patientprogramid = ppt.patientprogramid 				AND pa7.ActionId IN ( 					SELECT		 						DISTINCT pa8.ActionId 					FROM          						RPT_Patient AS pt8 with (nolock) INNER JOIN 						RPT_PatientProgram AS pp8 with (nolock)  ON pt8.PatientId = pp8.PatientId INNER JOIN 						RPT_PatientProgramModule AS pm8 with (nolock) ON pp8.PatientProgramId = pm8.PatientProgramId INNER JOIN 						RPT_PatientProgramAction AS pa8 with (nolock)  ON pm8.PatientProgramModuleId = pa8.PatientProgramModuleId INNER JOIN 						RPT_PatientProgramStep AS ps8 with (nolock) ON pa8.ActionId = ps8.ActionId LEFT OUTER JOIN 						RPT_PatientProgramResponse AS pr8 with (nolock) ON ps8.StepId = pr8.StepId 					WHERE   						((ps8.SourceId = '53f56eb7ac80d31203000001') OR (ps8.SourceId = '532c4061f8efe368860003b6') ) 						AND ((pa8.SourceId = '53f57115ac80d31203000014') OR (pa8.SourceId = '532c46b3c347865db8000092')) 						AND pp8.Name = 'BSHSI - Healthy Weight v2' 						AND (pp8.[Delete] = 'False') 						AND pa8.[State] IN ('Completed')	 						AND pt8.PatientId = pt7.PatientId 						AND pp8.patientprogramid = ppt.patientprogramid 				) 			ORDER BY pa7.ArchivedDate DESC	 			) 	)) as [Disenroll_Reason]	
			,( 		COALESCE( 			( 				SELECT		 	  					CASE WHEN pr9.[delete] = 'False' AND pa9.[State] = 'Completed' 						THEN 							CASE WHEN ISDATE(pr9.Value) = 1 THEN CAST(pr9.Value AS DATE) ELSE NULL END  						ELSE 							NULL 					END 						 					AS [didnotenrollValue]                    				FROM       					RPT_Patient AS pt9 WITH (NOLOCK) INNER JOIN 					RPT_PatientProgram AS pp9 WITH (NOLOCK) ON pt9.PatientId = pp9.PatientId INNER JOIN 					RPT_PatientProgramModule AS pm9 WITH (NOLOCK) ON pp9.PatientProgramId = pm9.PatientProgramId INNER JOIN 					RPT_PatientProgramAction AS pa9 WITH (NOLOCK) ON pm9.PatientProgramModuleId = pa9.PatientProgramModuleId INNER JOIN 					RPT_PatientProgramStep AS ps9 WITH (NOLOCK) ON pa9.ActionId = ps9.ActionId LEFT OUTER JOIN 					RPT_PatientProgramResponse AS pr9 WITH (NOLOCK) ON ps9.StepId = pr9.StepId 				WHERE      					pp9.Name = 'BSHSI - Healthy Weight v2' 					AND ps9.Question = 'Date did not enroll:'  					AND pa9.Name = 'Enrollment'  					AND (pp9.[Delete] = 'False')  					AND (pa9.Archived = 'False')  					AND pa9.ArchivedDate IS NULL 					AND pr9.Value != ''  					AND pa9.[State] IN ('Completed') 					AND pt9.patientId = pt.patientid 					AND pp9.patientprogramid = ppt.patientprogramid 			), 			( 			SELECT	TOP 1		  				CASE WHEN pr10.[delete] = 'False' AND pa10.[State] = 'Completed' 					THEN 						CASE WHEN ISDATE(pr10.Value) = 1 THEN CAST(pr10.Value AS DATE) ELSE NULL END  					ELSE 						NULL 				END 						 				AS [didnotenrollValuearch]                    			FROM       				RPT_Patient AS pt10 WITH (NOLOCK) INNER JOIN 				RPT_PatientProgram AS pp10 WITH (NOLOCK) ON pt10.PatientId = pp10.PatientId INNER JOIN 				RPT_PatientProgramModule AS pm10 WITH (NOLOCK) ON pp10.PatientProgramId = pm10.PatientProgramId INNER JOIN 				RPT_PatientProgramAction AS pa10 WITH (NOLOCK) ON pm10.PatientProgramModuleId = pa10.PatientProgramModuleId INNER JOIN 				RPT_PatientProgramStep AS ps10 WITH (NOLOCK) ON pa10.ActionId = ps10.ActionId LEFT OUTER JOIN 				RPT_PatientProgramResponse AS pr10 WITH (NOLOCK) ON ps10.StepId = pr10.StepId 			WHERE  				pp10.SourceId = '541943a6bdd4dfa5d90002da'     				AND ps10.Question = 'Date did not enroll:'  				AND pa10.Name = 'Enrollment'  				AND (pp10.[Delete] = 'False')  				AND (pa10.Archived = 'TRUE')  				AND pa10.ArchivedDate IS NOT NULL 				AND pr10.Value != ''  				AND pt10.patientid = pt.patientid	 				AND pa10.[State] IN ('Completed')	 				AND pp10.patientprogramid = ppt.patientprogramid 				AND pa10.ActionId IN ( 					SELECT		 						DISTINCT pa11.ActionId 					FROM          						RPT_Patient AS pt11 with (nolock) INNER JOIN 						RPT_PatientProgram AS pp11 with (nolock)  ON pt11.PatientId = pp11.PatientId INNER JOIN 						RPT_PatientProgramModule AS pm11 with (nolock) ON pp11.PatientProgramId = pm11.PatientProgramId INNER JOIN 						RPT_PatientProgramAction AS pa11 with (nolock)  ON pm11.PatientProgramModuleId = pa11.PatientProgramModuleId INNER JOIN 						RPT_PatientProgramStep AS ps11 with (nolock) ON pa11.ActionId = ps11.ActionId LEFT OUTER JOIN 						RPT_PatientProgramResponse AS pr11 with (nolock) ON ps11.StepId = pr11.StepId 					WHERE   						pp11.SourceId = '541943a6bdd4dfa5d90002da'    			 						AND ps11.Question = 'Date did not enroll:'  						AND pa11.Name = 'Enrollment'  						AND pp11.Name = 'BSHSI - Healthy Weight v2' 						AND (pp11.[Delete] = 'False') 						AND pa11.[State] IN ('Completed')	 						AND pt11.PatientId = pt10.PatientId 						AND pp11.patientprogramid = ppt.patientprogramid 				)	 			ORDER BY pa10.ArchivedDate DESC 			) 	)) as [did_not_enroll_date]
			,( 		COALESCE( 			(SELECT  					(CASE WHEN pr12.[delete] = 'False' AND pa12.[State] = 'Completed' 						THEN 							pr12.[Text] 					ELSE 						NULL 					END) AS [didnotenrollreasonText] 				FROM          					RPT_Patient AS pt12 with (nolock) INNER JOIN 					RPT_PatientProgram AS pp12 with (nolock) ON pt12.PatientId = pp12.PatientId INNER JOIN 					RPT_PatientProgramModule AS pm12 with (nolock) ON pp12.PatientProgramId = pm12.PatientProgramId INNER JOIN 					RPT_PatientProgramAction AS pa12 with (nolock) ON pm12.PatientProgramModuleId = pa12.PatientProgramModuleId INNER JOIN 					RPT_PatientProgramStep AS ps12 with (nolock) ON pa12.ActionId = ps12.ActionId LEFT OUTER JOIN 					RPT_PatientProgramResponse AS pr12 with (nolock) ON ps12.StepId = pr12.StepId 				WHERE   					pp12.sourceid = '541943a6bdd4dfa5d90002da'    					AND ((ps12.SourceId = '53f4f885ac80d30e00000065') OR (ps12.SourceId = '532c3fc2f8efe368860003b5') ) 					AND ((pa12.SourceId = '53f4fd75ac80d30e00000083') OR (pa12.SourceId = '532c45bff8efe36886000446') ) 					AND (pp12.[Delete] = 'False')  					AND (pa12.Archived = 'False')  					AND (pr12.Selected = 'True')  					AND pa12.ArchivedDate IS NULL 					AND pt12.patientid = pt.patientid 					AND pa12.[State] IN ('Completed') 					AND pp12.patientprogramid = ppt.patientprogramid 				) 		, 			(	SELECT TOP 1 					(CASE WHEN pr13.[delete] = 'False' AND pa13.[State] = 'Completed' 						THEN 							pr13.[Text] 					ELSE 						NULL 					END) AS [didnotenrollTextarch] 				FROM          					RPT_Patient AS pt13 with (nolock) INNER JOIN 					RPT_PatientProgram AS pp13 with (nolock) ON pt13.PatientId = pp13.PatientId INNER JOIN 					RPT_PatientProgramModule AS pm13 with (nolock) ON pp13.PatientProgramId = pm13.PatientProgramId INNER JOIN 					RPT_PatientProgramAction AS pa13 with (nolock) ON pm13.PatientProgramModuleId = pa13.PatientProgramModuleId INNER JOIN 					RPT_PatientProgramStep AS ps13 with (nolock) ON pa13.ActionId = ps13.ActionId LEFT OUTER JOIN 					RPT_PatientProgramResponse AS pr13 with (nolock) ON ps13.StepId = pr13.StepId 				WHERE   					pp13.sourceid = '541943a6bdd4dfa5d90002da'    					AND ((ps13.SourceId = '53f4f885ac80d30e00000065') OR (ps13.SourceId = '532c3fc2f8efe368860003b5') ) 					AND ((pa13.SourceId = '53f4fd75ac80d30e00000083') OR (pa13.SourceId = '532c45bff8efe36886000446') ) 					AND (pp13.[Delete] = 'False')  					AND (pa13.Archived = 'True')  					AND (pr13.Selected = 'True')  					AND pa13.ArchivedDate IS NOT NULL 					AND pa13.[State] IN ('Completed') 					AND pt13.PatientId = pt.patientId 					AND pp13.patientprogramid = ppt.patientprogramid 					AND pa13.ActionId IN ( 							SELECT  								DISTINCT pa14.ActionId 							FROM          								RPT_Patient AS pt14 with (nolock) INNER JOIN 								RPT_PatientProgram AS pp14 with (nolock) ON pt14.PatientId = pp14.PatientId INNER JOIN 								RPT_PatientProgramModule AS pm14 with (nolock) ON pp14.PatientProgramId = pm14.PatientProgramId INNER JOIN 								RPT_PatientProgramAction AS pa14 with (nolock) ON pm14.PatientProgramModuleId = pa14.PatientProgramModuleId INNER JOIN 								RPT_PatientProgramStep AS ps14 with (nolock) ON pa14.ActionId = ps14.ActionId LEFT OUTER JOIN 								RPT_PatientProgramResponse AS pr14 with (nolock) ON ps14.StepId = pr14.StepId 							WHERE   								pp14.sourceid = '541943a6bdd4dfa5d90002da'    								AND ((ps14.SourceId = '53f4f885ac80d30e00000065') OR (ps14.SourceId = '532c3fc2f8efe368860003b5') ) 								AND ((pa14.SourceId = '53f4fd75ac80d30e00000083') OR (pa14.SourceId = '532c45bff8efe36886000446') ) 								AND (pp14.[Delete] = 'False')  								AND (pr14.Selected = 'True')  								AND pt14.PatientId = pt13.patientId 								AND pa14.[State] IN ('Completed') 								AND pp14.patientprogramid = ppt.patientprogramid	 					)	 				ORDER BY pa13.ArchivedDate DESC	 			) 	)) as [did_not_enroll_reason]
		FROM
	...
	--SELECT * FROM vw_RPT_BSHSI_HW2_Enrollment_Info
END
...
BEGIN
	...
	TRUNCATE TABLE CareMemberTypeLookUp
	
	TRUNCATE TABLE ContactEmail
	TRUNCATE TABLE ContactPhone
	TRUNCATE TABLE ContactAddress
	TRUNCATE TABLE ContactRecentList
	TRUNCATE TABLE ContactMode
	TRUNCATE TABLE ContactLanguage
	TRUNCATE TABLE ContactWeekDay
	TRUNCATE TABLE ContactTimeOfDay

	-- todos
	TRUNCATE TABLE ToDoProgram
	TRUNCATE TABLE ToDo

	-- patient programs
	TRUNCATE TABLE SpawnElements
	TRUNCATE TABLE SpawnElementTypeCode
	TRUNCATE TABLE PatientProgramAttribute
	TRUNCATE TABLE PatientProgramResponse
	TRUNCATE TABLE PatientProgramStep
	TRUNCATE TABLE PatientProgramAction	
	TRUNCATE TABLE PatientProgramModule
	TRUNCATE TABLE PatientProgram	
	
	TRUNCATE TABLE PatientNoteProgram
	TRUNCATE TABLE PatientNote
	TRUNCATE TABLE PatientProblem

	TRUNCATE TABLE ObjectiveCategory
	TRUNCATE TABLE ObjectiveLookUp	
	TRUNCATE TABLE PatientObservation	
	TRUNCATE TABLE Observation
	TRUNCATE TABLE PatientTaskAttributeValue
	TRUNCATE TABLE PatientTaskAttribute
	TRUNCATE TABLE PatientTaskBarrier
	TRUNCATE TABLE PatientTask

	-- patient allergies
	TRUNCATE TABLE Allergy
	TRUNCATE TABLE AllergyType
	TRUNCATE TABLE PatientAllergy
	TRUNCATE TABLE PatientAllergyReaction
	
	-- patient medsupps
	TRUNCATE TABLE PatientMedSuppPhClass
	TRUNCATE TABLE PatientMedSuppNDC
	TRUNCATE TABLE PatientMedSupp	

	-- patient goal
	TRUNCATE TABLE PatientGoalProgram
	TRUNCATE TABLE PatientGoalFocusArea
	TRUNCATE TABLE GoalAttributeOption	
	TRUNCATE TABLE PatientGoalAttributeValue
	TRUNCATE TABLE PatientGoalAttribute
	TRUNCATE TABLE GoalAttribute
	
	TRUNCATE TABLE PatientInterventionBarrier
	TRUNCATE TABLE PatientIntervention	
	TRUNCATE TABLE PatientBarrier	
	TRUNCATE TABLE PatientGoal
	TRUNCATE TABLE PatientUser	
	TRUNCATE TABLE Contact

	TRUNCATE TABLE PatientSystem

	TRUNCATE TABLE Patient
	
	TRUNCATE TABLE CommTypeCommMode
	TRUNCATE TABLE ToDoCategoryLookUp	
	TRUNCATE TABLE RPTMongoCategoryLookUp
	TRUNCATE TABLE SourceLookUp
	TRUNCATE TABLE BarrierCategoryLookUp
	TRUNCATE TABLE InterventionCategoryLookUp
	TRUNCATE TABLE RPTMongoTimeZoneLookUp
	TRUNCATE TABLE ProblemLookUp
	TRUNCATE TABLE TimesOfDayLookUp
	TRUNCATE TABLE CommTypeLookUp
	TRUNCATE TABLE CommModeLookUp
	TRUNCATE TABLE StateLookUp
	TRUNCATE TABLE LanguageLookUp
	TRUNCATE TABLE FocusAreaLookUp
	TRUNCATE TABLE CodingSystemLookUp
	TRUNCATE TABLE ObservationTypeLookUp
	TRUNCATE TABLE AllergyTypeLookUp
	TRUNCATE TABLE AllergySourceLookUp
	TRUNCATE TABLE SeverityLookUp
	TRUNCATE TABLE ReactionLookUp
	TRUNCATE TABLE MedSupTypeLookUp
	TRUNCATE TABLE FreqHowOftenLookUp
	TRUNCATE TABLE FreqWhenLookUp
	
	
	TRUNCATE TABLE UserRecentList
	TRUNCATE TABLE [User]
	
	--TRUNCATE TABLE CohortPatientView	
	--TRUNCATE TABLE CohortPatientViewSearchField

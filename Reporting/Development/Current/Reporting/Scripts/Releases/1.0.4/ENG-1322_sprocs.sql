...
CREATE PROCEDURE [dbo].[spPhy_RPT_SaveMaritalStatusLookUp] 
	@MongoId varchar(50),
...
BEGIN
	...
	If Exists(Select Top 1 1 From RPT_MaritalStatusLookUp Where MongoId = @MongoId)
	Begin
		Update RPT_MaritalStatusLookUp
		Set 
	...
	Else
	Begin
		Insert Into RPT_MaritalStatusLookUp(
			LookUpType, 
			Name, 
			[Default], 
			MongoId, 
...
CREATE PROCEDURE [dbo].[spPhy_RPT_SaveStatusReasonLookUp] 
	@MongoId varchar(50),
	@LookUpType varchar(MAX),
	@Name varchar(MAX),
	@Active varchar(50),
...
BEGIN
	...
	If Exists(Select Top 1 1 From RPT_StatusReasonLookUp Where MongoId = @MongoId)
	Begin
		Update RPT_StatusReasonLookUp
		Set 
			LookUpType = @LookUpType,
			Name = @Name,
			[Default] = @Default,
	...
	Else
	Begin
		Insert Into RPT_StatusReasonLookUp(
			LookUpType, 
			Name, 
			[Default], 
			MongoId, 
...
CREATE PROCEDURE [dbo].[spPhy_RPT_SavePatient]
	...
	@MongoMaritalStatusId varchar(50),
	@Protected varchar(50),
	@Deceased varchar(50),
	@Status varchar(50),
	@MongoReasonId varchar(50),
	@StatusDataSource varchar(50)
AS
BEGIN
	-- SET NOCOUNT ON added to prevent extra result sets from
	-- interfering with SELECT statements.
	...
	Begin
		...
		Set FirstName = @FirstName,
			...
			MongoMaritalStatusId = @MongoMaritalStatusId,
			Protected = @Protected,
			Deceased = @Deceased,
			[Status] = @Status,
			MongoReasonId = @MongoReasonId,
...
AS
	...
	SET Minor = (CASE WHEN AGE is null THEN 'No' WHEN AGE < 18 THEN 'No' WHEN AGE >= 18 THEN 'Yes' END)
GO

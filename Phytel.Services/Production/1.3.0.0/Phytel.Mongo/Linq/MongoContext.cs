...
namespace Phytel.Mongo.Linq
{
    ...
    public abstract class MongoContext : IDisposable
    {
        ...
        {
            _connectionStringBuilder = new MongoUrlBuilder(Phytel.Services.MongoService.Instance.GetConnectionString(database, isContract, asAdmin, systemType));
        ...
        {
            _connectionStringBuilder = new MongoUrlBuilder(Phytel.Services.MongoService.Instance.GetConnectionString(dbConnName, database, isContract, asAdmin, systemType));
            _databaseName = _connectionStringBuilder.DatabaseName;
            _client = new MongoClient(_connectionStringBuilder.ToMongoUrl());
        }
        #endregion
